	...
	if servingInfo != nil {
		if len(clientCAFile) > 0 {
			...
			if newServingInfo != nil {
				...
				glog.Infof("new port: must avoid %v %v", avoid, ok)
				for _, cert := range clientCAs {
					if ok && cert.Subject.CommonName == avoid {
						// Ensure the old CA doesn't leak out the new port.
						glog.Infof("new port: skipping client ca %v", cert.Subject.CommonName)
						continue
					}
					glog.Infof("new port: adding client ca %v", cert.Subject.CommonName)
					newServingInfo.ClientCA.AddCert(cert)
				}
			}
			avoid, ok := newServingInfo.GetCertIssuerCommonName()
			glog.Infof("old port: must avoid %v %v", avoid, ok)

			for _, cert := range clientCAs {
				if ok && cert.Subject.CommonName == avoid {
					// Ensure the new CA doesn't leak out the old port.
					glog.Infof("old port: skipping client ca %v", cert.Subject.CommonName)
					continue
				}
				glog.Infof("old port: adding client ca %v", cert.Subject.CommonName)
