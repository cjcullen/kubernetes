	...
	*config = &server.SecureServingInfo{
	...
	if newConfig != nil && s.OldTLSPort > 0 {
		if s.NewTLSCertFile == "" || s.NewTLSPrivateKeyFile == "" {
			glog.Fatalf("--old-tls-port was specified, so --new-tls-cert-file and --new-tls-private-key-file are required.")
		}
		glog.Infof("Configuring TLS migration mode. Old stack on port %v, new stack on port %v.", s.OldTLSPort, s.BindPort)
		n := **config
