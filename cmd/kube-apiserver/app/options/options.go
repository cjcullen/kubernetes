...
func NewAPIServer() *APIServer {
	s := APIServer{
		...
		WebhookTokenAuthnCacheTTL: 30 * time.Second,
	}
	return &s
}
...
func (s *APIServer) AddFlags(fs *pflag.FlagSet) {
	...
	fs.DurationVar(&s.WebhookTokenAuthnCacheTTL, "authentication-token-webhook-cache-ttl", s.WebhookTokenAuthnCacheTTL, "The duration to cache responses from the webhook token authenticator. Default is 30s")
