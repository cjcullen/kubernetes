...
function start-node-problem-detector {
  ...
  if [[ -z "${flags}" ]]; then
    ...
    local -r custom_km_config="${KUBE_HOME}/node-problem-detector/config/kernel-monitor-counter.json,${KUBE_HOME}/node-problem-detector/config/systemd-monitor-counter.json"
...
function setup-etcd-encryption {
  ...
  # FALSE CLOUD_KMS_INTEGRATION implies that etcd encryption feature is not enabled.
  # However, we can't just exit; still need to remove encryption related placeholders from the kube-apiserver.manifest.
  if [[ "${CLOUD_KMS_INTEGRATION:-FALSE}" == FALSE ]]; then
    echo "etcd level encryption will not be enabled since CLOUD_KMS_INTEGRATION is set to FALSE."
    sed -i -e " {
  ...
  echo "Enabling etcd level encryption via CloudKMS Plugin."
