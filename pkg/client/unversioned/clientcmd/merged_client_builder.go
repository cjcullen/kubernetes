	...
	if config.clientConfig == nil {
		...
		if config.clientConfig == nil {
			...
			if config.fallbackReader != nil {
				mergedClientConfig = NewInteractiveClientConfig(*mergedConfig, config.overrides.CurrentContext, config.overrides, config.fallbackReader, PersistAuthConfigForUser(config.loadingRules))
			} else {
				mergedClientConfig = NewNonInteractiveClientConfig(*mergedConfig, config.overrides.CurrentContext, config.overrides, PersistAuthConfigForUser(config.loadingRules))
			}
